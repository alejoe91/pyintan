...
from .tools import read_qstring, plural, read_python, get_rising_falling_edges
...
class File:
    ...
    def __init__(self, filename):
        ...
        header_size = 0
        data_dtype = None
        if self._absolute_filename.endswith('.rhs'):
            self._global_info, self._ordered_channels, data_dtype, \
            header_size, self._block_size = read_rhs(self._absolute_filename)
        elif self._absolute_filename.endswith('.rhd'):
            self._global_info, self._ordered_channels, data_dtype, \
            header_size, self._block_size = read_rhd(self._absolute_filename)

        print(data_dtype, header_size)
        # memmap raw data with the complicated structured dtype
        self._raw_data = np.memmap(self._absolute_filename, dtype=data_dtype, mode='r', offset=header_size)
        ...
        self.sig_channels = np.array(sig_channels, dtype=_signal_channel_dtype)

        self._max_sampling_rate = np.max(self.sig_channels['sampling_rate'])
        self._max_sigs_length = self._raw_data.size * self._block_size

        shape = self._raw_data[self.sig_channels[0]['name']].shape
        size = self._raw_data[self.sig_channels[0]['name']].size
        ...
        anas_chan = [sig for sig in self.sig_channels if 'ANALOG' not in sig['name'] and 'STIM' not in sig['name']]
        anas_len = len(anas_chan)
        stim_chan = [sig for sig in self.sig_channels if 'STIM' in sig['name']]
        stim_len = len(stim_chan)
        analog_in_chan = [sig for sig in self.sig_channels if 'ANALOG-IN' in sig['name']]
        analog_in_len = len(analog_in_chan)
        analog_out_chan = [sig for sig in self.sig_channels if 'ANALOG-OUT' in sig['name']]
        analog_out_len = len(analog_out_chan)

        self.anas = np.zeros((i_stop - i_start, anas_len), dtype='uint16')
        stim_data = np.zeros((i_stop - i_start, stim_len), dtype='uint16')
        self.analog_in = np.zeros((i_stop - i_start, analog_in_len), dtype='uint16')
        self.analog_out = np.zeros((i_stop - i_start, analog_out_len), dtype='uint16')

        for i, ch in enumerate(anas_chan):
            data_chan = self._raw_data[ch['name']]
            if len(shape) == 1:
                self.anas[:, i] = data_chan[i_start:i_stop]
            else:
                self.anas[:, i] = data_chan[block_start:block_stop].flatten()[sl0:sl1]
        ...
        for i, ch in enumerate(analog_in_chan):
            data_chan = self._raw_data[ch['name']]
            if len(shape) == 1:
                self.analog_in[:, i] = data_chan[i_start:i_stop]
            else:
                self.analog_in[:, i] = data_chan[block_start:block_stop].flatten()[sl0:sl1]

        for i, ch in enumerate(analog_out_chan):
            data_chan = self._raw_data[ch['name']]
            if len(shape) == 1:
                self.analog_out[:, i] = data_chan[i_start:i_stop]
            else:
                self.analog_out[:, i] = data_chan[block_start:block_stop].flatten()[sl0:sl1]

        # DIGITAL
        try:
            data_chan = self._raw_data['DIGITAL-IN']
            if len(shape) == 1:
                self.digital_in = data_chan[i_start:i_stop]
            else:
                self.digital_in = data_chan[block_start:block_stop].flatten()[sl0:sl1]
        except:
            print('No Digital IN')
            self.digital_out = []
        ...
        try:
            ...
            if len(shape) == 1:
                self.digital_out = data_chan[i_start:i_stop]
            else:
                self.digital_out = data_chan[block_start:block_stop].flatten()[sl0:sl1]
        except:
        ...
        if np.count_nonzero(stim_data) != 0:
            ...
            stim_polarity = np.zeros(stim_data.shape, dtype='uint16')
            ...
            # self.stim_channels = np.array(stim_channels)
            # self.stim_signal = np.array(stim_signal)
            self.stim_data_proc = stim_data
...
if __name__ == '__main__':
    file = '/home/alessiob/Documents/Data/rhs_tests/bipolarstim_15uA_170418_192136.rhs'
